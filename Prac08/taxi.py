...
class SilverServiceTaxi(Car):
    ...
    def __init__(self, name, fuel):
        ...
        self.price_per_km = 1.20
        self.current_fare_distance = 0
        self.fanciness = 2*self.price_per_km
        self.flagfall = float(4.50)
        self.fare = 0

    def __str__(self):
        """ return a string representation like a car but with current fare distance"""
        return "{}, {}km on current fare, ${:.2f}/km plus flagfall of ${:.2f} Price is ${:.2f}".format(super().__str__(), self.current_fare_distance, self.fanciness, self.flagfall, self.fare)
